...
public class VarDefinitionHelper {
  ...
  public void setVarDefinitions() {
    ...
    // RTF: sort variable declarations by ORIGINAL bytecode slot.
    // Disabled: causes 1 regression and doesn't help TextureDraw.Create
    // because the issue is dup store order, not declaration order.
    // if (DecompilerContext.isRoundtripFidelity()) {
  ...
  /**
   * Final post-pass: fix any remaining duplicate variable definitions that the
   * renaming pass missed. Two patterns:
   * 1. Same VVP defined twice (nameMap.put overwrites, no clash detected)
   *    → remove the second definition
   * 2. Different VVPs with same name in same scope (nameMap cleared between siblings)
   *    → rename the second variable
   */
  private void fixRemainingDuplicateDefinitions(Statement root) {
    // Walk the entire statement tree via getSequentialObjects() (rendering order).
    // Track all defined variable names. When a duplicate is found, rename it.
    Set<String> seenNames = new HashSet<>();
    walkSequentialObjects(root, seenNames);
  }

  private void walkSequentialObjects(Statement stat, Set<String> names) {
    // Process varDefinitions
    for (Exprent vd : stat.getVarDefinitions()) {
      scanExprentForDefs(vd, names);
    }

    // Process exprents (for leaf BasicBlockStatements)
    if (stat.getExprents() != null) {
      for (Exprent expr : stat.getExprents()) {
        scanExprentForDefs(expr, names);
      }
    }

    // Each child statement gets its own scope
    for (Statement child : stat.getStats()) {
      walkSequentialObjects(child, new HashSet<>(names));
    }
  }

  private void scanExprentForDefs(Exprent expr, Set<String> names) {
    // Only check top-level: bare VarExprent definition or AssignmentExprent LHS definition
    VarExprent var = null;
    ...
    if (var != null) {
      ...
      if (name != null && !names.add(name)) {
        ...
        names.add(newName);
        this.varproc.setClashingName(var.getVarVersionPair(), newName);
      }
    }
  }

  private void checkDefForDuplicate(Exprent expr, Set<String> names) {
    VarExprent var = null;
    if (expr instanceof VarExprent && ((VarExprent) expr).isDefinition()) {
      var = (VarExprent) expr;
    } else if (expr instanceof AssignmentExprent) {
    ...
    // Also check sub-expressions for definitions (e.g., for-each init, try-with-resources)
    if (var == null) {
      for (Exprent sub : expr.getAllExprents(true)) {
        if (sub instanceof VarExprent && ((VarExprent) sub).isDefinition()) {
          checkDefForDuplicate(sub, names);
    ...
    if (!names.add(name)) {
      // Duplicate in same scope - rename
      String newName = name + "x";
      while (names.contains(newName)) newName += "x";
      names.add(newName);
      this.varproc.setClashingName(var.getVarVersionPair(), newName);
  ...
  private void checkAndFixDuplicateDef(Exprent expr, Set<String> names) {
    if (!(expr instanceof VarExprent)) return;
    ...
    if (!names.add(name)) {
      // Duplicate name in this scope. Rename the second one using clashing names
      // (which override LVT names in VarExprent.getName()).
      String newName = name + "x";
   ...
   */
  private void sortDeclsByOriginalSlot(Statement stat) {
    // Sort varDefinitions
    List<Exprent> varDefs = stat.getVarDefinitions();
    if (varDefs.size() > 1) {
      varDefs.sort((a, b) -> Integer.compare(getOrigSlot(a), getOrigSlot(b)));
    }

    // Sort leading safe declarations in exprents
    if (stat.getExprents() != null) {
      sortLeadingDeclsByOrigSlot(stat.getExprents());
    }

    for (Statement child : stat.getStats()) {
      sortDeclsByOriginalSlot(child);
    }
  }

  private void sortLeadingDeclsByOrigSlot(List<Exprent> exprents) {
    // Collect indices of safe-to-reorder declarations within the leading
    // declaration block. Unsafe declarations (with dependencies) stay fixed;
    // safe ones are sorted around them.
    int declEnd = 0;
    List<Integer> safeIndices = new ArrayList<>();
    for (int i = 0; i < exprents.size(); i++) {
      Exprent expr = exprents.get(i);
      if (!isAnyDeclaration(expr)) break;
      declEnd = i + 1;
      if (isSafeToReorderDecl(expr)) {
        safeIndices.add(i);
      }
    }
    if (safeIndices.size() <= 1) return;

    // Extract safe declarations, sort by original slot, reinsert
    List<Exprent> safeDecls = new ArrayList<>();
    for (int idx : safeIndices) safeDecls.add(exprents.get(idx));
    safeDecls.sort((a, b) -> Integer.compare(getOrigSlot(a), getOrigSlot(b)));
    for (int i = 0; i < safeIndices.size(); i++) {
      exprents.set(safeIndices.get(i), safeDecls.get(i));
    }
  }

  private static boolean isAnyDeclaration(Exprent expr) {
  ...
  private static boolean isSafeToReorderDecl(Exprent expr) {
    ...
    if (expr instanceof AssignmentExprent ae && ae.getLeft() instanceof VarExprent ve && ve.isDefinition()) {
      // Safe if RHS is a constant (no dependency on other locals)
      return ae.getRight() instanceof ConstExprent;
    }
    return false;
  }

  /** Get the original bytecode slot for a variable declaration. */
  private int getOrigSlot(Exprent expr) {
    int idx = getVarIndex(expr);
    if (idx == Integer.MAX_VALUE) return Integer.MAX_VALUE;
    return varproc.getOriginalVarIndex(idx);
  }
