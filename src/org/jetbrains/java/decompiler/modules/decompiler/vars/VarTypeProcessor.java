...
public class VarTypeProcessor {
  ...
  private static void collectUsageTypesFromExpr(Exprent expr, Map<VarVersionPair, VarType> minTypes,
    ...
    // Check if an Object-typed variable is the RHS of an assignment to a typed LHS.
    // E.g., IsoGridSquare square1 = var0; -> var0 should be IsoGridSquare, not Object.
      ...
      if (assign.getRight() instanceof VarExprent) {
        ...
        if (rhsType != null && "java/lang/Object".equals(rhsType.value)) {
          ...
          if (lhsType.typeFamily == TypeFamily.OBJECT
              && lhsType.type == CodeType.OBJECT
              && !"java/lang/Object".equals(lhsType.value)) {
            varAssignTypes.computeIfAbsent(rhsVar.getIndex(), k -> new ArrayList<>())
                .add(lhsType);
          }
        }
      }
